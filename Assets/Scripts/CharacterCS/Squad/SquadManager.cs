...
public class SquadManager : MonoBehaviour
{
    ...
    public void StartPatrolMode()
    {
        ...
        _currentLeader.StateManager.ChangeState(AIState.Patrol);

        // 班員は班長追従状態に設定
        foreach (var unit in _spawnedUnits)
        {
            if (unit != null && unit != _currentLeader && !unit.Combat.IsDead)
            {
                // 班員は移動状態で班長を追従
                unit.StartFollowingLeader();
                unit.StateManager.ChangeState(AIState.Move);
            }
        }
    }
    ...
    public void StartDefendMode()
    {
        ...
        foreach (var unit in _spawnedUnits)
        {
            if (unit != null && !unit.Combat.IsDead)
            {
